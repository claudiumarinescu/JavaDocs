...
public class FiltersAndMapExercise {
    ...
    public static  List<Integer> findEvenNumbers() {
        ...
        evenNumbers = input.filter(x -> (x & 1) == 0).collect(toList());
    ...
    public static List<String> findPersonByName(String name){
        ...
        personName = input.filter(s -> s.equals(name)).collect(toList());
    ...
    public static List<Person> findPersonByAgeAndName(List<Person> personList, String name, Integer age){
        ...
        persons = personList.stream().filter(s -> (s.getAge() == age) && (s.getName().equals(name))).collect(toList());
    ...
    public static List<Integer> doubleNumbers() {
        ...
        doubleNumbers = input.map(x -> 2 * x).collect(toList());
    ...
    public static List<String> getOnlyNames(List<Person> personList){
        ...
        //TODO #6
        // HINT: use map
        collect = personList.stream().map(s -> s.getName()).collect(toList());
    ...
    public static int findTheMinimumNumber(){
        ...
        // HINT: use reduce
        int min = 0;
        min = input.reduce(Integer.MAX_VALUE, Integer::min);
...
//        int min = 0;
//        min = personList.stream().map(p -> p.getAge()).reduce(Integer.MAX_VALUE, Integer::min);
//        return min;
//    }

    public static int findTheYoungestPerson(List<Person> personList){
        // same as point 12, only with Collectors
        int min = 0;
        min = personList.stream().collect(minBy(Comparator.comparingInt(d -> d.getAge()))).get().getAge();
        return min;
    }
    ...
    public static double findAgeAverage(List<Person> personList){
        double sum = 0;
        //TODO #13
        sum = personList.stream().map(p -> p.getAge()).reduce(0, Integer::sum);
    ...
    public static Optional<Integer> findSquareDivisibleBy3(){
        ...
        firstSquareDivisibleByThree = someNumbers.stream()
                .filter(x -> (x % 3) == 0)
                .filter(x -> Math.pow(Math.sqrt(x), 2) == x )
                .findFirst();
